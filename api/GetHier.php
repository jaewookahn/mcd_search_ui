...
while ($ms = MySQL_fetch_assoc($res)) { //build array that will be json encoded 
	...
	array_push($s, $ms); // build array of results
	
}

echo ' ' . $s[0]['conceptA_ID'] . '<br>';
empty($ms);	
empty($s);
...
$res=MySQL_query($sql) or print('There was an error');

while ($ms = MySQL_fetch_assoc($res)) { //build array that will be json encoded 
	array_push($s, $ms); // build array of results, allows for building json
}
empty($ms);	
echo ' -- ' . $s[0]['conceptB_ID'] . '<br>';
/* End PHP */
?> 